...
export async function createBooking(booking: BookingData): Promise<{ success: boolean; message: string; bookingId?: string; contractUrl?: string; paymentUrl?: string }> {
  try {
    ...
    if (!contractUrl) {
      ...
      contractUrl = `https://docuseal.eu/d/NfUmr9QnzPYYsd?email=${emailEncoded}&nombre_arrendador=${nombre}&dni=${dni}&nombre_acompa%C3%B1ante=${nombreAcomp}&dni_acompa%C3%B1ante=${dniAcomp}&servicios_contratados=${encodeURIComponent(jornadaNameStr)}&numero_personas=${numPersonas}&fecha_entrada=${encodeURIComponent(fechaEntrada.split(" ")[0].split('-').reverse().join('/'))}&hora_entrada=${encodeURIComponent((firstJornadaTime || '').substring(0,5))}&fecha_salida=${encodeURIComponent(fechaSalida.split(" ")[0].split('-').reverse().join('/'))}&hora_salida=${encodeURIComponent((lastJornadaTime || '').substring(0,5))}`;
    }
